...
export function setupColorTokens(colorFrame) {
  if (colorFrame) {
    ...
    colorFrame.children.forEach(color => {
      const colorString = `rgba(${color.fills[0].color.r * 255}, ${color
        .fills[0].color.g * 255}, ${color.fills[0].color.b * 255}, ${color
        .fills[0].color.a * 1})`;
    ...
    return colors;
  } else console.error("No frame for setupColorTokens()!");
}
