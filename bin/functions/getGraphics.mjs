...
import { getFromApi } from './getFromApi.mjs';
import { downloadFile } from './downloadFile.mjs';

import { errorGetGraphics, errorGetGraphicsNoImages, errorGetIds } from '../meta/errors.mjs';
 ...
 */
export async function getGraphics(graphicsPage, config) {
  ...
  if (IMAGE_RESPONSE.err) throw new Error(errorGetGraphics);
  if (!IMAGE_RESPONSE.images) throw new Error(errorGetGraphicsNoImages);
