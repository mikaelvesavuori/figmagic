...
async function getData(recompileLocal, outputFolderBaseFile, outputFileName, token, url) {
    try {
        ...
        const DATA = (async () => {
            if (recompileLocal)
                return getDataLocal_1.getDataLocal(outputFolderBaseFile, outputFileName);
            else {
                if (token && url)
